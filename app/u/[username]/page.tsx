...
export default async function PublicUserPage({
  ...
  const [cardsResponse, decksResponse] = await Promise.all([
    supabase
      ...
      .select(
        "id,name,image,set_name,set_code,collector_number,language,foil,quantity,price",
      )
  ...
  const displayCards = safeCards.slice(0, 120);
  const safeDecks = decksResponse.data || [];

  const totalQuantity = safeCards.reduce(
    (sum, card) => sum + Number(card.quantity || 1),
    0,
  );

  const totalValue = safeCards.reduce(
          ...
          <section className="mt-6 rounded-[2rem] border border-white/10 bg-white/[0.055] p-5 shadow-xl">
            ...
            <div className="mt-5 grid gap-3 md:grid-cols-2 xl:grid-cols-3">
              {safeDecks.map((deck) => {
                ...
                const winrate =
                  games > 0
                    ? Math.round((Number(deck.wins || 0) / games) * 100)
                    : 0;

                return (
                  <div
                    key={deck.id}
                    className="rounded-[1.6rem] border border-white/10 bg-black/25 p-3"
                  >
                    <div className="flex items-center gap-3">
                      {deck.commander_image ? (
                        ...
                        <div className="flex h-20 w-14 items-center justify-center rounded-xl bg-white/10 text-xl">
                          🎴
                        </div>
                      )}
                      ...
                      <div className="min-w-0 flex-1">
                        ...
                        <p className="truncate text-sm font-bold text-[#f59e0b]">
                          {deck.commander || "Commandant inconnu"}
                        </p>
                        <p className="mt-1 text-xs font-bold text-white/45">
                          {deck.colors || "Incolore"}
                        </p>
                      </div>
                    </div>
                    ...
                    <div className="mt-3 grid grid-cols-3 gap-2 text-center">
                      ...
                      <MiniStat
                        label="Prix"
                        value={formatCurrency(Number(deck.price || 0))}
                      />
                    </div>
                  </div>
                );
          ...
          <div className="flex items-center justify-between gap-3">
            <div>
              ...
              <p className="mt-1 text-sm font-bold text-white/45">
                Collection complète calculée • aperçu des 120 dernières cartes.
              </p>
            </div>
          </div>
          ...
          {displayCards.length > 0 ? (
            <div className="mt-4 grid grid-cols-3 gap-3 sm:grid-cols-4 md:grid-cols-6 xl:grid-cols-8">
              {displayCards.map((card) => (
                <div
                  key={card.id}
                  className="rounded-2xl border border-white/10 bg-white/[0.055] p-1.5 shadow-xl"
                >
                  {card.image ? (
                    <img
                      src={card.image}
                    ...
                    <div className="flex aspect-[63/88] items-center justify-center rounded-xl bg-black/30 text-2xl">
                      🎴
                    </div>
                  )}
            ...
            <div className="mt-5 rounded-[2rem] border border-white/10 bg-white/[0.055] p-8 text-center">
              ...
              <p className="mt-2 font-bold text-white/45">
                Ce joueur n’a pas encore de collection publique.
              </p>
            </div>
          )}
        </section>
